...
export default function UtilitiesScreen() {
  ...
  const pay = () => {
    ...
    if (!account) {
      show(`Enter your ${selected.accountLabel.toLowerCase()}`, 'error');
      return;
    }
    setStage('processing');
    haptics.processing();
    ...
    <ScreenContainer>
      ...
      <StaggeredItem index={1}>
        <View style={styles.grid}>
          {utilities.map((u) => (
            <PressableScale
              key={u.id}
              style={[styles.utilCard, { backgroundColor: theme.surface, borderColor: selected.id === u.id ? theme.accent : theme.border }]}
              onPress={() => { setSelected(u); setAccount(u.savedAccount || ''); haptics.selection(); }}
            >
              <View style={[styles.utilIcon, { backgroundColor: u.color + '22' }]}>
                <Text style={styles.utilEmoji}>{u.icon}</Text>
              </View>
      ...
      <SlideUpModal visible={stage === 'processing'} onClose={() => {}}>
        <View style={styles.center}>
          <ShimmerLoader />
          <Text style={[styles.centerText, { color: theme.text }]}>Processing payment…</Text>
        </View>
      </SlideUpModal>
