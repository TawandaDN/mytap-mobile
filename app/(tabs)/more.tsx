...
export default function MoreScreen({ onOpenGuardrail, onOpenAssistant }: { onOpenGuardrail: () => void; onOpenAssistant: () => void }) {
  const { theme } = useTheme();
  const { state } = useApp();
  const { show } = useToast();
  const profile = { name: 'Tawanda Moko', phone: '+267 71 234 567', email: 'tawanda@mytap.bw', tier: 'Gold' };
  const menuItems = [
  ...
  return (
    <ScreenContainer>
      ...
      <StaggeredItem index={1}>
        <GlassCard bubbleColor="rgba(107,58,138,0.15)">
          <View style={styles.profileRow}>
            <View style={[styles.avatar, { backgroundColor: theme.accent }]}><Text style={styles.avatarText}>T</Text></View>
            ...
            <View style={[styles.tierBadge, { backgroundColor: '#F5A623' }]}><Text style={styles.tierText}>{profile.tier}</Text></View>
          </View>
        </GlassCard>
      </StaggeredItem>
      <StaggeredItem index={2}>
        <PressableScale onPress={() => { haptics.medium(); onOpenGuardrail(); }}>
          <GlassCard bubbleColor="rgba(46,204,113,0.15)">
            <View style={styles.guardrailRow}>
              <View style={[styles.guardrailIcon, { backgroundColor: '#2ECC71' }]}><Ionicons name="shield-checkmark" size={20} color="#fff" /></View>
              <View style={styles.guardrailInfo}>
                <Text style={[styles.guardrailTitle, { color: theme.text }]}>MyTap Guardrail</Text>
                <Text style={[styles.guardrailSub, { color: theme.textMuted }]}>P{guardrailProfile.used.toLocaleString()} of P{guardrailProfile.monthlyLimit.toLocaleString()} used</Text>
              </View>
      ...
      <StaggeredItem index={3}>
        <PressableScale onPress={() => { haptics.medium(); onOpenAssistant(); }}>
          <GlassCard bubbleColor="rgba(52,152,219,0.15)">
            <View style={styles.guardrailRow}>
              <View style={[styles.guardrailIcon, { backgroundColor: '#3498DB' }]}><Ionicons name="sparkles" size={20} color="#fff" /></View>
              <View style={styles.guardrailInfo}>
                <Text style={[styles.guardrailTitle, { color: theme.text }]}>MyTap Assistant</Text>
                <Text style={[styles.guardrailSub, { color: theme.textMuted }]}>Ask about your spending, data & more</Text>
              </View>
              <Ionicons name="chevron-forward" size={18} color={theme.textMuted} />
            </View>
          </GlassCard>
      ...
      <StaggeredItem index={4}>
        ...
        <GlassCard bubble={false}>
          {menuItems.map((item, i) => (
            <PressableScale key={item.label} style={[styles.menuRow, i > 0 && styles.menuDivider]} onPress={() => { haptics.light(); item.onPress(); }}>
      ...
      <StaggeredItem index={5}>
        <PressableScale style={[styles.signOut, { borderColor: '#E74C3C' }]} onPress={() => { haptics.warning(); show('Signed out (demo)', 'info'); }}>
...
const styles = StyleSheet.create({
  ...
  guardrailIcon: { width: 44, height: 44, borderRadius: 22, alignItems: 'center', justifyContent: 'center' },
  guardrailInfo: { flex: 1 },
  guardrailTitle: { fontSize: 16, fontWeight: '600' },
  guardrailSub: { fontSize: 13, marginTop: 2 },
