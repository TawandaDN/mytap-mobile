    ...
    <ScreenContainer>
      <StaggeredItem index={0}>
        <View style={styles.header}>
          <View>
            <Text style={[styles.greeting, { color: theme.textMuted }]}><Typewriter text={`${greeting},`} speed={30} /></Text>
            <Text style={[styles.name, { color: theme.text }]}>Tawanda</Text>
          </View>
          <View style={styles.headerIcons}>
            <PressableScale style={styles.iconBtn} onPress={() => haptics.light()}><Ionicons name="notifications-outline" size={20} color={theme.text} /></PressableScale>
            <PressableScale style={styles.iconBtn} onPress={() => haptics.light()}><Ionicons name="settings-outline" size={20} color={theme.text} /></PressableScale>
          </View>
        </View>
      </StaggeredItem>
      <StaggeredItem index={1}><CardCarousel cards={state.cards} /></StaggeredItem>
      <StaggeredItem index={2}>
        <View style={styles.quickRow}>
          {quickActions.map((a) => (
            <PressableScale key={a.label} style={styles.quickItem} onPress={() => haptics.medium()}>
              <View style={[styles.quickIcon, { backgroundColor: a.color + '22' }]}><Ionicons name={a.icon as any} size={22} color={a.color} /></View>
      ...
      <StaggeredItem index={3}>
        <GlassCard style={styles.dayCard} bubbleColor="rgba(245,166,35,0.2)">
          <View style={styles.dayHeader}>
            ...
            <View style={styles.dayBadge}><Text style={styles.dayBadgeText}>Ready</Text></View>
          </View>
      ...
      <StaggeredItem index={5}>
        <GlassCard bubble={false}>
          ...
          <View style={styles.progressTrack}><View style={[styles.progressFill, { width: `${guardrailProfile.pct}%`, backgroundColor: theme.accent }]} /></View>
          <Text style={[styles.guardrailText, { color: theme.textMuted }]}>P{guardrailProfile.used.toLocaleString()} used of P{guardrailProfile.monthlyLimit.toLocaleString()}</Text>
        </GlassCard>
      </StaggeredItem>
    </ScreenContainer>
  );
...
const styles = StyleSheet.create({
  ...
  quickRow: { flexDirection: 'row', justifyContent: 'space-between', marginTop: spacing.xl, marginBottom: spacing.xl },
  ...
  dayCard: { marginBottom: spacing.xl },
  dayHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  dayTitle: { fontSize: 18, fontWeight: '700' },
  daySub: { fontSize: 13, marginTop: 2 },
  dayBadge: { backgroundColor: '#2ECC71', paddingHorizontal: 12, paddingVertical: 5, borderRadius: 999 },
  dayBadgeText: { color: '#fff', fontSize: 12, fontWeight: '600' },
  dayReward: { marginTop: spacing.lg },
  dayRewardLabel: { fontSize: 13 },
  dayRewardValue: { fontSize: 32, fontWeight: '700' },
  ...
  guardrailHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: spacing.md },
  guardrailPct: { fontSize: 20, fontWeight: '700' },
  progressTrack: { height: 8, borderRadius: 4, backgroundColor: 'rgba(15,23,41,0.08)', overflow: 'hidden' },
  progressFill: { height: '100%', borderRadius: 4 },
  guardrailText: { fontSize: 13, marginTop: spacing.sm },
});