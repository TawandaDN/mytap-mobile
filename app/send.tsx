...
export default function SendScreen() {
  ...
  const send = () => {
    ...
    setTimeout(() => {
      dispatch({ type: 'PAY', cardId: 'wallet', amount: amt, merchant: contact.name, category: 'Send money', icon: '💸', color: '#1ABC9C', method: 'MyTap Wallet' });
      const r = { id: `r-${Date.now()}`, merchant: contact.name, category: 'Send money', amount: amt, date: new Date().toISOString(), ref: `MT-${Math.floor(100000 + Math.random() * 900000)}`, method: 'MyTap Wallet', status: 'completed' as const, icon: '💸', color: '#1ABC9C' };
    ...
    <ScreenContainer>
      ...
      <StaggeredItem index={1}>
        ...
        <GlassCard bubble={false}>
          {contacts.map((c, i) => (
            <PressableScale key={c.id} style={[styles.contactRow, i > 0 && styles.divider]} onPress={() => { setContact(c); haptics.selection(); }}>
              <View style={[styles.avatar, { backgroundColor: c.avatarColor }]}><Text style={styles.avatarText}>{c.name[0]}</Text></View>
              <View style={styles.contactInfo}><Text style={[styles.contactName, { color: theme.text }]}>{c.name}</Text><Text style={[styles.contactPhone, { color: theme.textMuted }]}>{c.phone}</Text></View>
      ...
      <SlideUpModal visible={stage === 'processing'} onClose={() => {}}><View style={styles.center}><Ionicons name="sync" size={40} color={theme.accent} /><Text style={[styles.centerText, { color: theme.text }]}>Sending…</Text></View></SlideUpModal>
      <SlideUpModal visible={stage === 'success'} onClose={() => setStage('idle')}>
        <Confetti active={stage === 'success'} />
        <View style={styles.center}><View style={[styles.successCircle, { backgroundColor: '#2ECC71' }]}><Ionicons name="checkmark" size={36} color="#fff" /></View><Text style={[styles.successTitle, { color: theme.text }]}>Money sent!</Text><Text style={[styles.successAmount, { color: theme.text }]}>{receipt ? formatPula(receipt.amount) : ''}</Text><Text style={[styles.successTo, { color: theme.textMuted }]}>to {contact.name}</Text></View>
