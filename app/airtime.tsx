...
export default function AirtimeScreen() {
  ...
  const buy = () => {
    ...
    setTimeout(() => {
      ...
      const r = { id: `r-${Date.now()}`, merchant: `${network.name} Airtime`, category: 'Airtime', amount: amt, date: new Date().toISOString(), ref: `MT-${Math.floor(100000 + Math.random() * 900000)}`, method: 'MyTap Wallet', status: 'completed' as const, icon: network.icon, color: network.color };
    ...
    <ScreenContainer>
      ...
      <StaggeredItem index={1}>
        <View style={styles.grid}>
          {airtimeNetworks.map((n) => (
            <PressableScale key={n.id} style={[styles.netCard, { backgroundColor: theme.surface, borderColor: network.id === n.id ? theme.accent : theme.border }]} onPress={() => { setNetwork(n); haptics.selection(); }}>
              <View style={[styles.netIcon, { backgroundColor: n.color + '22' }]}><Text style={styles.netEmoji}>{n.icon}</Text></View>
      ...
      <StaggeredItem index={2}>
        <GlassCard bubble={false}>
          ...
          <View style={[styles.inputWrap, { borderColor: theme.border }]}><TextInput value={phone} onChangeText={setPhone} keyboardType="phone-pad" placeholder="+267 7X XXX XXX" placeholderTextColor={theme.textMuted} style={[styles.input, { color: theme.text }]} /></View>
          <Text style={[styles.fieldLabel, { color: theme.textMuted }]}>Amount</Text>
          <View style={styles.quickAmounts}>
            {[10, 20, 30, 50, 100].map((a) => (
              <PressableScale key={a} style={[styles.quickAmount, { backgroundColor: theme.surfaceAlt }]} onPress={() => { setAmount(String(a)); haptics.light(); }}><Text style={[styles.quickAmountText, { color: theme.textSecondary }]}>P{a}</Text></PressableScale>
            ))}
          </View>
...
const styles = StyleSheet.create({
  ...
  netCard: { width: '47%', borderRadius: radius.lg, borderWidth: 1, padding: spacing.lg, alignItems: 'center', gap: 6 },
  netIcon: { width: 48, height: 48, borderRadius: 24, alignItems: 'center', justifyContent: 'center' },
  netEmoji: { fontSize: 22 },
  netName: { fontSize: 15, fontWeight: '600' },
